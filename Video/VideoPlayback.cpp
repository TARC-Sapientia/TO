...
{	
VideoPlayback::VideoPlayback(unsigned int frameStreamId, unsigned int videoFramesMaxLength) :
...
void VideoPlayback::pushVideoFrame(boost::shared_array<char> data, const unsigned int dataLength) {
    ...
    if (frameThrown)
        cout << "Communication: FrameStream " << m_frameStreamId << ": decode list full. Oldest frame thrown." << endl;
}
